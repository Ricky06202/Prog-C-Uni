...
void determinarValores(bool automatico)
{
...
void procesarLosDatosYGuardarlos()
{
   ...
   while ((int)leerParametros(&CANTIDAD_DE_ALUMNOS, &CANTIDAD_DE_SEMESTRES, &CANTIDAD_DE_MATERIAS) == 1)
   {
      determinarValores(true);
   }
   struct alumno
   {
      char nombre[25];
...
void crearArchivo()
{
   ...
   determinarValores(false);
   while ((int)leerParametros(&cantidadAlumnos, &cantidadSemestres, &cantidadMaterias) == 1)
   {
